...
abstract contract ITravelPBMManager {
    ...
    struct PBMToken {
        ...
        // Total supply
        uint256 totalSupply;
        // Percent support for payment (4 decimals)
        uint256 percentSupport;
    }
    ...
    function getTokenDetails(
        uint256 tokenId
    ) external view virtual returns (PBMToken memory pbmToken_);

    function increaseTotalSupply(uint256 tokenId, uint256 amount) external virtual;
    function decreaseTotalSupply(uint256 tokenId, uint256 amount) external virtual;
    function sovToPBM(uint256 tokenId, uint256 sovAmount) external virtual view returns (uint);
    function pbmToSov(uint256 tokenId, uint256 pbmAmount) external virtual view returns (uint);
